...
export async function GET(req: Request) {
    try {
        logger.info('Admin Jobs API: GET Request received');
        const session = await verifyAuth(req)
        if (!session || !ALLOWED_ADMIN_ROLES.includes(session.user.role)) {
            logger.warn('Admin Jobs API: Unauthorized access attempt');
        ...
        logger.info(`Admin Jobs API: Returning ${jobs.length} jobs`);
