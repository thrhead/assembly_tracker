...
async function checkCalendarQuery() {
    ...
    console.log('Testing Prisma Query (JS)...');
    ...
    try {
        ...
        if (jobs.length > 0) {
            ...
            const mapped = jobs.map(job => {
                const companyName = job.customer ? job.customer.company : 'Müşteri Yok';
                return {
                    id: job.id,
                    title: `${companyName} - ${job.title}`,
                    // Check if assignments map works
                    assignments: job.assignments ? job.assignments.map(a => a.id) : []
                }
            });
